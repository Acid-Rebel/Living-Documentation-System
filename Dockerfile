...
COPY frontend/package.json frontend/package-lock.json frontend/next.config.ts frontend/tsconfig.json frontend/tailwind.config.js frontend/postcss.config.mjs ./
...
COPY --from=py_deps /core/.venv /opt/venv
COPY core/ ./core/
...
RUN echo '#!/bin/bash\n\
    ...
    python -m core.main --port ${PORT:-8001} &\n\
