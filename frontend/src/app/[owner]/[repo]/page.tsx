...
export default function RepoWikiPage() {
  ...
  const [loadingMessage, setLoadingMessage] = useState<string | undefined>(
    messages.loading?.initializing || 'Initializing documentation generation...'
  );
  ...
  // File tree paths (newline-delimited) from the GitHub/GitLab/Bitbucket API
...
4. Return ONLY valid XML with the structure specified above, with no markdown code block delimiters`
  ...
  const fetchRepositoryStructure = useCallback(async () => {
    ...
    try {
      ...
      // Store file tree for the sidebar file browser
      setFileTreePaths(fileTreeData);
  ...
  const exportWiki = useCallback(async (format: 'markdown' | 'json') => {
    ...
    try {
      setIsExporting(true);
      setExportError(null);
      setLoadingMessage(`${language === 'ja' ? 'ドキュメントを' : 'Exporting docs as '} ${format} ${language === 'ja' ? 'としてエクスポート中...' : '...'}`);
  ...
  const confirmRefresh = useCallback(async (newToken?: string) => {
    ...
    setLoadingMessage(messages.loading?.initializing || 'Initializing documentation generation...');
          ...
          <div className="h-full overflow-y-auto flex flex-col lg:flex-row gap-4 w-full overflow-hidden bg-[var(--card-bg)] rounded-lg shadow-custom card-japanese">
            ...
            <div className="h-full w-full lg:w-[280px] xl:w-[320px] flex-shrink-0 bg-[var(--background)]/50 rounded-lg rounded-r-none p-5 border-b lg:border-b-0 lg:border-r border-[var(--border-color)] overflow-y-auto">
              ...
              <div className="mb-3 flex items-center text-xs text-[var(--muted)]">
                <span className="mr-2">Docs Type:</span>
              ...
              <div className="mb-5">
                ...
                >
                  ...
                  {messages.repoPage?.refreshWiki || 'Refresh Docs'}
                </button>
              </div>
              ...
              {Object.keys(generatedPages).length > 0 && (
                <div className="mb-5">
                  <h4 className="text-sm font-semibold text-[var(--foreground)] mb-3 font-serif">
                    {messages.repoPage?.exportWiki || 'Export Docs'}
                  </h4>
              ...
              <div className="flex border-b border-[var(--border-color)] mb-3 flex-shrink-0 mt-4">
                <button
                  className={`flex-1 py-1.5 text-xs font-semibold border-b-2 transition-colors ${leftTab === 'modules' ? 'border-[var(--accent-primary)] text-[var(--accent-primary)]' : 'border-transparent text-[var(--muted)] hover:text-[var(--foreground)]'}`}
                  onClick={() => setLeftTab('modules')}
                >
                  Modules
                </button>
                <button
                  className={`flex-1 py-1.5 text-xs font-semibold border-b-2 transition-colors ${leftTab === 'files' ? 'border-[var(--accent-primary)] text-[var(--accent-primary)]' : 'border-transparent text-[var(--muted)] hover:text-[var(--foreground)]'}`}
              ...
              <div className="flex-1 overflow-y-auto min-h-0 pr-1 pb-4">
                {leftTab === 'modules' ? (
                  ...
                  fileTreePaths ? (
                    <FileTreeView
                      filePaths={fileTreePaths}
                      onFileClick={(path) => {
                        const url = generateFileUrl(path);
                        if (url && url !== path) window.open(url, '_blank');
                      }}
                    />
                  ) : (
                    <div className="text-sm text-[var(--muted)] p-4 mt-2 text-center border border-dashed border-[var(--border-color)] rounded-md flex flex-col items-center">
                      <FaFolder className="text-2xl mb-2 opacity-50" />
                      Generate docs first to see the file tree.
                    </div>
                  )
                )}
              </div>
            ...
            {/* Main Content Area */}
            <div id="main-content" className="w-full flex-grow flex flex-col overflow-hidden bg-[var(--card-bg)]">
              {/* Tabs */}
              <div className="flex border-b border-[var(--border-color)] bg-[var(--background)]/30 overflow-x-auto custom-scrollbar flex-shrink-0">
                {[
                  ...
                  <button
                    ...
                    className={`flex items-center gap-2 px-4 py-3 text-sm font-medium whitespace-nowrap border-b-2 transition-colors ${mainTab === tab.id ? 'border-[var(--accent-primary)] text-[var(--accent-primary)] bg-[var(--accent-primary)]/5' : 'border-transparent text-[var(--muted)] hover:text-[var(--foreground)] hover:bg-[var(--background)]/50'}`}
                  >
                    <tab.icon /> {tab.label}
                  </button>
                ))}
              ...
              <div className="flex-1 overflow-y-auto p-6 lg:p-8">
                {mainTab === 'documentation' && (
                  currentPageId && generatedPages[currentPageId] ? (
                    ...
                    <div className="flex flex-col items-center justify-center p-8 text-[var(--muted)] h-full min-h-[400px]">
                      ...
                      <p className="font-serif text-center relative z-10">
                ...
                {mainTab === 'semantic' && (
                  <SemanticInsightsPanel repoInfo={effectiveRepoInfo} />
                )}
                ...
                {mainTab === 'ask' && (
                  <div className="h-full min-h-[600px] w-full max-w-4xl mx-auto flex flex-col border border-[var(--border-color)] rounded-lg overflow-hidden relative">
                    <Ask
      ...
      <footer className="max-w-[90%] xl:max-w-[1400px] mx-auto mt-8 flex flex-col gap-4 w-full">
        <div className="flex justify-between items-center gap-4 text-center text-[var(--muted)] text-sm h-fit w-full bg-[var(--card-bg)] rounded-lg p-3 shadow-sm border border-[var(--border-color)]">
          <p className="flex-1 font-serif">
            {messages.footer?.copyright || 'Living Documentation System - Generate Docs from GitHub/Gitlab/Bitbucket repositories'}
          </p>
          <ThemeToggle />
        </div>
      </footer>
